...
- **18 of the 24 shipped federal recipes derive from partner-developed content, so their `us_federal_public_domain` basis is unproven** (`rust/crates/food-domain/content/starter_recipes.json`, every entry whose `provenance.rights.attribution` names *Deliciously Healthy Dinners* or *Deliciously Healthy Family Meals*) -- AC-2's second evidence item, the fresh-context rights review in section A of `docs/research/mvp-032-verifier-prompt.txt`, was run 2026-09-07 against the live source pages and returned **18 FAIL / 6 PASS / 0 NOT VERIFIED**. The failure is not the authorship wording, original or amended: it is the exclusion neither wording relaxes, a collection developed with a named non-federal partner. Both cookbooks say so in their own NHLBI-published acknowledgments -- "Recipes were developed by David Kamen ... Chef/Instructor at the Culinary Institute of America, and Colleen Pierre ... consultant", tested by Northern Illinois University (NIH Pub 10-2921, verbatim-verified this session; 10-7531 carries the same credit). 17 USC 105 reaches works of federal officers and employees, and neither publication prints an assignment or a copyright notice, so the basis these 18 ship on is not established. The research pass missed it by reading the publication *landing pages*, which name no developer, instead of the publications -- the same hazard that correctly rejected *Honoring the Gift of Heart Health*. Five further entries pass as worded but are flagged: their collections credit a non-federal party with *testing* or *review* rather than development (Columbia University for the Latino and African American books). Only `minestrone-soup` is clean on substance, and its authorship-trace URL now serves an unrelated page. Fix: an owner decision, not a code change -- drop the 18 (shipped roster 49 -> 31, and the AC-4 coverage counts need re-deriving), re-base them on a rights claim that does not assert federal authorship, or seek NHLBI clarification. Recorded as an amendment on the `MVP-032` `Done` row in `docs/ROADMAP.md`, which downgrades AC-2 from `PASS` to `FAIL`. Blocks nothing mechanically; it is a rights exposure on already-shipping content and should be settled before the next release build.
  **Status:** OPEN
