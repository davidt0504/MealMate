...
- **No federal-source recipe ships pending written rights clearance** (`docs/research/MVP-032_STARTER_PROVENANCE_MANIFEST.json`) -- D-043 quarantined the eighteen records whose collection credits identify contractor development; D-044 applies the same rule to the remaining six rather than treating publisher presence as sufficient rights proof. All twenty-four complete records remain outside the APK for provenance and future review. Existing installs archive/hide them, block restore, preserve history, and remove future occurrences. The 25 owner-reviewed recipes remain sufficient for the starter MVP; fresh-install, existing-install, and Cover evidence remain to be re-run.
  **Status:** IN PROGRESS 2026-09-09 (MVP-032 / D-043 / D-044)
...
- **A pantry mark re-applied after being removed does not re-hide a line the user previously restored** (`lib/features/shopping/shopping_screen.dart:161`, cf. `lib/features/shopping/shopping_copy.dart:175`) -- residual of the `restored`-flag finding fixed in the same pass. `_setLine` now clamps `restored` to `false` unless the line's status is `omittedPantryMarked`, so an inert flag is dropped by the next write to that line; but the sequence *Add anyway → unmark the ingredient in Pantry → re-mark it* performs no line-state write between the unmark and the re-mark, so the clamp never fires and the stored `restored: true` still routes the line to **To buy** via `sectionFor`. The user sees a pantry mark that appears to have had no effect; the explain sheet's "Back to pantry" does render in that state and clears it. Closing it properly needs the pantry write path to clear `restored` for the ingredient's `m:` line keys — a cross-feature write into `shopping_line_state` coupled to the MVP-015 key format, on a card already approved Done — or a mark-generation stamp on the overlay row. It is also a defensible product reading that "Add anyway" is a durable statement of intent that a later mark should not silently overturn; that question should be settled before either implementation. Full review: `~/.claude/reviews/redteam-impl-handoff-orch-31-2026-08-29T2331-d2ca.md`
  **Status:** RESOLVED 2026-09-13 (FIX-001 item 2) — AC-2 settled the product reading: "Add anyway" holds against the mark it overrode, and a newly set mark re-hides. `set_pantry_mark_in` (`rust/crates/kimatta-storage/src/lib.rs`) now clears `restored` on the identity's `m:` overlay rows in every window whenever a mark is newly set, using `food_domain::line_key_prefix`, which `GroupKey::line_key` also builds from. Known limit: overflow-fallback `s:` lines carry no identity in their key and are not cleared.
...
- **MVP-011 AC-3 (human cook log) is discharged only as cook reviews are recorded** (`docs/research/MVP-032_STARTER_PROVENANCE_MANIFEST.json`, `docs/ROADMAP.md` MVP-011 Done row) -- approved Done 2026-08-29 under D-027 with 0/10 recipes reviewed. **Partly resolved 2026-09-05 by MVP-032, further 2026-09-06, and narrowed by the 2026-09-09 D-043 rights quarantine:** D-041's two-armed rule ships an entry with a recorded `cook_review` *or* with `rights.basis == us_federal_public_domain` and a `source_url`. **Six cleared NHLBI entries** ship on the rights arm and **25 owner household entries** ship on the cook arm, so a fresh install holds 31 recipes and is no longer empty. **Read the remaining ten carefully: `basis: original` no longer means "not shipped".** The owner's 25 are also `original` and do ship, on a recorded `cook_review`. What is still pending is the **ten Kimatta-authored entries** — the ones whose `provenance.source_author` is `Kimatta` rather than `David` — which carry `cook_review: null` and ship by neither arm. Fix, for those ten only: cook each and add `"cook_review": {"cooked_on": "YYYY-MM-DD", "by": "...", "corrections": null}`. Only one remaining federal source collection (*Heart Healthy Home Cooking African American Style*, 3 of the 6 federal entries) states its recipes were tested; the rest ship on publisher standing, which is the residual risk D-041 accepted.
...
- **`GLUTEN_TERMS` has no term for a pasta shape, and five sibling misses share its fix** (`rust/crates/food-domain/src/restriction.rs`, `GLUTEN_TERMS`, `DAIRY_TERMS`, `FISH_TERMS`; `RULE_VERSION` at `:135`) -- rule version 1 matches `"pasta"`, `"spaghetti"` and `"noodles"` but no shape: not `macaroni`, `rotini`, `orzo`, `ravioli`, `ziti`, `farfalle`, `rigatoni`, `linguine` or `cavatappi`. A content drop that names a line by its shape alone therefore ships a wheat dish gluten-free *by declaration*, and `every_recipe_matches_its_declared_conflicts` passes because the declaration matches what `assess` returns. That is how `crockpot-mac-and-cheese`, `sausage-and-broccoli-orzo` and `taco-pasta` shipped on 2026-09-06; they were closed by naming the lines accurately (see `policy.expected_conflicts_note`), which is a content fix and does not generalise. `ravioli-bake` still declares gluten only through its separate "pasta sauce" line, pinned in the negative by `ravioli_bake_declares_gluten_only_through_its_sauce_line`. Fix: add the shape terms, `RULE_VERSION` 1 -> 2. The same bump is where every other recorded rule-version-1 miss belongs, so settle them together rather than spending the version bump on one. The full set, re-derived against the file with the real matcher on 2026-09-07 and mirrored one-for-one in `policy.expected_conflicts_note` and in `every_line_name_declares_its_known_allergen_words`: `ravioli` matching no gluten term (`ravioli-bake`); soy sauce matching no gluten term, on **five** entries, not the two the note named until this pass -- `peanut-noodle-bowl` and `vegetable-fried-rice` unshipped, `steamed-salmon-and-mushrooms`, `pineapple-chicken-skewers` and `egg-roll-in-a-bowl` shipped; `"ranch dressing"`/`"ranch seasoning"` matching no dairy term (`crockpot-barbecue-chicken`, `crockpot-chicken-tacos`); and three found on 2026-09-07 -- `worcestershire sauce` matching no **fish** term (`barbecued-chicken`, `easy-shepherds-pie`), and `cream of chicken soup` (`crockpot-sausage-and-hashbrown-casserole`, `smothered-pork-chops`, `taco-soup`) and `gravy mix` (`smothered-pork-chops`) matching no gluten term. So the bump touches three tables, not one, and `every_term_matches_its_own_kind` hard-codes a length for each: `Gluten` 15, `Dairy` 18 and `Fish` 13 all have to move with it. The bump also shifts the AC-4 coverage counts recorded in the `MVP-032` evidence row in `docs/ROADMAP.md` (gluten-free 31, vegan 7, vegetarian 10), which is cheaper to know now than to discover while closing the bump card. Unlike a content rename, a matcher change reaches already-installed devices, because assessments derive at read time rather than being stored. Revisit before `MVP-022` beta readiness.
  **Status:** OPEN
