...
- **Current implementation task:** `MVP-004` — `Verify`; awaiting the Elevated-tier fresh-context verifier and owner status approval.
- **Next implementation task:** none — no register row is `Ready`. The implementation lane is occupied by `MVP-004` at `Verify`; `MVP-005` becomes the unique `Ready` row, and this line restates it, on `MVP-004`'s `Verify → Done` transition. (`docs/task/README.md`: the **Next implementation task** is the unique `Ready` row in the register.)
...
| MVP-004 | Verify | Anonymous-first entry and household identity | MVP-002, MVP-003 |
