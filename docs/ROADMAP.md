...
- **Milestone:** EMULATOR-PERSISTENCE-READY
- **Current implementation task:** `MVP-002` — `Verify`; implementation and all four acceptance criteria are complete, but explicit owner approval of the `Verify → Done` transition is outstanding.
- **Next implementation task:** `MVP-003` — blocked until `MVP-002` is explicitly promoted to `Done`; then it becomes `Ready` for `plan-task` without `--auto`.
...
| MVP-002 | Verify | Rust household kernel and SQLite foundation | PRE-002, DEC-005 |
...
- **Test↔toolchain coupling (DEC-005 decision 3):** `flutter test` runs the build hook, so every Dart test — the fake-fed widget test included — needs rustup + the pinned toolchain on the host; the widget test avoids *loading* the library, not *building* it. Additionally the generated loader looks in `rust/target/release/` under `flutter test`, hence the `&&`-chained `cargo build --release` command above (recorded as fix #1 in the PRE-002 ledger; no generated or integrate-written file was edited).
