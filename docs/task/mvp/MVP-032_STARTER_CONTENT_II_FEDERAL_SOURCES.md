...
- Write `docs/research/starter-candidates.md`: one row per candidate, roughly 25–30 rows, with the rubric columns — federal author verified (literal-yes/trace-only/no + source line + authorship-trace URL), servings ≥4, prep ≤30 min, ingredient-line count (recorded, **not a threshold** — relaxed by owner decision 2026-09-05, since nothing in the architecture reads it except shopping-list length), common-pantry ingredients, restriction kinds the dish triggers (vegan / vegetarian / gluten / dairy / nuts / fish / shellfish / soy / eggs / sesame), catalog additions needed, unit conversions needed, notes. Add an empty `owner decision` column and a summary line showing how many candidates cover each of vegan, vegetarian, gluten-free and omnivore.
