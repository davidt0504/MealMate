...
## Post-MVP intake and fix batches (D-042, 2026-09-08)

- `docs/BACKLOG.md` is the intake point for beta feedback: every note lands there verbatim, dated, with a source, and is triaged into exactly one disposition (bug dossier, `FIX` card, `OPT` card, folded into an existing card, or won't do). It carries no status.
- `FIX-nnn` cards (`docs/task/fix/`) bundle several bounded, root-caused fixes with no open design question into one session. Recommended workflow is `plan-task`. An item that turns out to need a decision is ejected back to the backlog, never resolved inside the plan.
- Design-sized work stays an `OPT-nnn` scoping card whose **Decision gates** section is the agenda for a dedicated `grill-me` session. Recommended workflow is `grill-me` then `plan-task`; planning may not start until every gate records an owner answer folded into the card.
- Lifecycle, gates, single implementation lane and handoff rules above apply unchanged to both kinds.
