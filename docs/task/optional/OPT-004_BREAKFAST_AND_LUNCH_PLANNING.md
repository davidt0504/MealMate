...
- **`prep_minutes` means what MVP-032 decided it means.** Breakfast slots will have short windows, so the field's existing ambiguity (prep-only vs prep+cook, `KNOWN_ISSUES.md`) bites harder here. Resolve it in this card or state why not. **Resolving it is a fill-in, not a research task (checked 2026-09-06):** both numbers already exist in the repo. Federal prep and cook are publisher-stated per row in `docs/research/starter-candidates.md` §1; owner cook is on the cards in `refs/owner-recipe-transcriptions.md`, which also records the derived prep/cook table for all 25 owner entries — clean for 22, with `davids-chili`, `broccoli-soup` and `ravioli-bake` flagged as needing an owner answer. Note also that the 2026-09-06 rule reached only 4 of the 24 federal entries; 17 still carry the older prep-only convention, so a straight lift of today's values into an `active` field would carry that inconsistency forward. See `policy.prep_minutes` in the content file for the per-entry scope.
