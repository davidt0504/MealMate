...
- **Discharge of the preceding criterion, added 2026-09-04 under D-040 (existing bullets byte-unchanged):** the pre-upload re-check is carried by `MVP-031` (`docs/task/mvp/MVP-031_PRODUCTION_ACTIVATION_AND_SUBMISSION.md`), AC-3, which re-verifies production application-ID availability immediately before the first upload that burns it and asserts no `.temp` identifier reaches the submission artefacts. That upload no longer "sits outside the register": `MVP-031` is a readiness card in the register whose acceptance shape follows `MVP-022:61` — it produces the packet and records owner-performed actions, and never performs the submission itself, so invariant 15 still holds. The alternate application-ID **shape** fallback above is unchanged and remains the remedy.
